...
export interface StateNewsGroup {
  state: string;
  stateName: string;
  stateColor: string;
  sourceUrl: string;
...
function isImmigrationRelated(text: string): boolean {
...
function parseRSSFeed(
  ...
  for (const block of blocks) {
    ...
    const dateStr = pubDate ? (() => {
      try { return new Date(pubDate).toISOString().split('T')[0]; } catch { return ''; }
    })() : '';

    items.push({
      id: `${state}-${simpleId(link || title)}`,
      state,
      stateName,
      title,
      summary: summary + (summary.length === 220 ? '…' : ''),
      url: link,
      date: dateStr,
    });
...
function parseHTMLPage(
  ...
  // Try several patterns to find linked headlines in news pages
  const patterns = [
    // <h2><a href="...">Title</a></h2> or <h3>
    /<h[2-4][^>]*>\s*<a[^>]+href="([^"#][^"]*)"[^>]*>([\s\S]*?)<\/a>/gi,
    // <a href="...">Title</a> with news/article class
  ...
  for (const pattern of patterns) {
    ...
    for (const match of matches) {
      ...
      items.push({
        id: `${state}-${simpleId(title)}`,
        state,
        stateName,
        title,
        summary: 'Visit the official state immigration website for full details.',
        url: href,
        date: '',
      });
...
const SOURCES: SourceConfig[] = [
  {
    ...
    stateColor: '#003DA5',
    sourceUrl: 'https://www.nsw.gov.au/immigration',
    rssUrl: 'https://www.nsw.gov.au/rss/news',
  ...
  },
  {
    ...
    stateName: 'Victoria',
    stateColor: '#003DA5',
    sourceUrl: 'https://www.vic.gov.au/immigration-news',
    rssUrl: 'https://www.vic.gov.au/rss.xml',
    filterByKeywords: true,
  },
  {
    state: 'QLD',
    ...
    stateColor: '#7B1C1C',
    sourceUrl: 'https://www.qld.gov.au/about/news-publications/news',
    htmlUrl: 'https://www.qld.gov.au/about/news-publications/news',
    filterByKeywords: true,
  },
  {
    state: 'WA',
    ...
    stateColor: '#1A5276',
    sourceUrl: 'https://www.migration.wa.gov.au/news',
    htmlUrl: 'https://www.migration.wa.gov.au/news',
  ...
  },
  {
    ...
    stateColor: '#C8102E',
    sourceUrl: 'https://www.migration.sa.gov.au/news-and-events',
    htmlUrl: 'https://www.migration.sa.gov.au/news-and-events',
    filterByKeywords: false,
  },
  {
    state: 'TAS',
    stateName: 'Tasmania',
    stateColor: '#1E8449',
    sourceUrl: 'https://www.migration.tas.gov.au/news',
    htmlUrl: 'https://www.migration.tas.gov.au/news',
    filterByKeywords: false,
  },
  {
    state: 'ACT',
    ...
    stateColor: '#5B2C6F',
    sourceUrl: 'https://www.act.gov.au/migration/news',
    htmlUrl: 'https://www.act.gov.au/migration/news',
    filterByKeywords: false,
  },
  {
    state: 'NT',
    ...
    stateColor: '#E07B00',
    sourceUrl: 'https://theterritory.com.au/migrate',
    htmlUrl: 'https://theterritory.com.au/migrate',
    filterByKeywords: false,
  },
];

async function fetchSource(source: SourceConfig): Promise<StateNewsGroup> {
  try {
...
export async function fetchAllImmigrationNews(): Promise<StateNewsGroup[]> {
  const results = await Promise.allSettled(SOURCES.map(fetchSource));
  return results.map((r, i) => {
    if (r.status === 'fulfilled') return r.value;
    return {
      state: SOURCES[i].state,
      stateName: SOURCES[i].stateName,
      stateColor: SOURCES[i].stateColor,
      sourceUrl: SOURCES[i].sourceUrl,
      items: [],
      error: true,
    };
  });
